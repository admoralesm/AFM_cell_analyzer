...
def _candidate_masks(gray, sensitivity, view_w, view_h, strip_lines):
    ...
    # 3: edge magnitude, which catches cells that match the background level
...
def detect_cell(
    ...
    if best is None:
        return {"found": False, "reason": "no candidate of a plausible size and shape"}
...
def annotate(frame_rgb, detection, label=None, color=(255, 60, 60), thickness=None):
    """Draw the detected outline, bounding box and a label onto a copy."""
    _require_cv2()
    ...
    try:
        for index in indices:
            ...
            if not ok:
                ...
                continue
            det = detect_cell(
                cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), roi=roi, sensitivity=sensitivity
            )
